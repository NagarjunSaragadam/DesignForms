$(document).ready(function () {
    ...
    $('#header_Project').change(function () {

        var Projval = document.getElementById('header_Project').value

        $.ajax({
            type: "Get",
            url: "/AssetRequisition/EmployeesbyProject",
            data: { Project: Projval },
            datatype: "json",
            traditional: true,
            success: function (data) {
                var Employees = "<select id='header_RequestedBy'>";
                var Employees = Employees + '<option value="">--Select--</option>';
                for (var i = 1; i < data.length; i++) {
                    Employees = Employees + '<option value="' + data[i].value + '">' + data[i].text + '</option>';
                }
                Employees = Employees + '</select>';

                var ApprovedEmployees = "<select id='header_ApprovedBy'>";
                var ApprovedEmployees = ApprovedEmployees + '<option value="">--Select--</option>';
                for (var i = 1; i < data.length; i++) {
                    ApprovedEmployees = ApprovedEmployees + '<option value="' + data[i].value + '">' + data[i].text + '</option>';
                }
                ApprovedEmployees = ApprovedEmployees + '</select>';
                $('#header_RequestedBy').html(Employees);
                $('#header_ApprovedBy').html(ApprovedEmployees);
