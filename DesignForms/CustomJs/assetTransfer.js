$(document).ready(function () {
    ...
    $('.notifications.btn').on('click', function (e) {
        ...
        $.validator.addMethod(
            ...
            'Must be greater then Requested From Date'
        );
        ...
        $('#dynamicform').validate({
            ...
            rules: {
                ...
                'header.FromProject': { required: true, },
                'header.ToProject': { required: true, },
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data?.success) {
                            ...
                            redirect();
                            notify(nFrom, nAlign, nIcons, nType, nAnimIn, nAnimOut,  message, " Asset Transfer Type ");
                        } else {
                            nType = 'danger';
                            message = data?.message;                                                        
                            notify(nFrom, nAlign, nIcons, nType, nAnimIn, nAnimOut, "  Asset Transfer Type  ");
                        }
                    },
...
function toJson() {
    var header ={
        Guid: $("#hdnGuid").val(), AssetRequisitionNo: $('#header_AssetTrRequisitionNo').val(), AssetRequisitionDate: $('#header_AssetTrRequisitionDate').val(),
        TaskType: $('#header_TaskType').val(), FromProject: $('#header_FromProject').val(), ToProject: $('#header_ToProject').val(), RequiredFromDate: $('#header_RequiredFromDate').val(),
    ...
    var details =[];
    $('#assetReqTbl > tbody  > tr').each(function () {
        var AssetTransferDetail = {};            
           if (this.cells.length > 0) {
               AssetTransferDetail['StructureType'] = this.cells[1].querySelector('input').value;
               AssetTransferDetail['StructureSubType'] = this.cells[2].querySelector('input').value;
               AssetTransferDetail['AssetType'] = this.cells[3].querySelector('input').value;
               AssetTransferDetail['AssetSpecification'] = this.cells[4].querySelector('input').value;
               AssetTransferDetail['QuantityRequired'] = this.cells[5].innerText;
               AssetTransferDetail['Uom'] = this.cells[6].querySelector('input').value;
               details.push(AssetTransferDetail);
           }        
        });
...
function redirect() {
    setTimeout(function () {        
        window.location.replace('/AssetTransfer/');
    }, 2000);
}
